  ...
  private:
    bool read_state(std::deque<int> buffer) {
      ...
      else if (buffer[0] >= 2 && buffer[0] <= 5)
      {
        ...
        if (height != id(height_sensor).state)
        {
          id(height_sensor).publish_state(height);
          ESP_LOGD("height", "%.1fcm", id(height_sensor).state);
        }
      }
